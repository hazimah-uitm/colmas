                    ...
                    <tbody>
                        @if (count($activities) > 0)
                            @foreach ($activities as $activityLog)
                                <tr>
                                    <td class="text-center">{{ ($activities->currentPage() - 1) * $activities->perPage() + $loop->iteration }}
                                    </td>
                                    <td>{{ $activityLog->description }}</td>
                                    <td>
                                        @if ($activityLog->subject)
                                            {{ class_basename($activityLog->subject_type) }} (ID:
                                                ...
                                                @foreach ($activityLog->properties['attributes'] as $key => $value)
                                                    @if (is_array($value))
                                                        @foreach ($value as $subKey => $subValue)
                                                            <li><strong>{{ ucfirst($key) }} ({{ $subKey }}):</strong>
                                                                {{ $subValue }}</li>
                                                        @endforeach
                                                    @else
