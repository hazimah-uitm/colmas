    ...
    <div class="card-body">
        <form method="POST" action="{{ $save_route }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('lab_management_id') is-invalid @enderror" id="lab_management_id" name="lab_management_id" value="{{ $computerLabName }}" disabled>
            ...
            <div class="mb-3" id="computerNameContainer">
                ...
                <input type="text" class="form-control @error('computer_name') is-invalid @enderror" id="computer_name" name="computer_name" value="{{ old('computer_name', $computerName) }}" {{ old('entry_option', $defaultEntryOption) == 'automatik' ? 'disabled' : '' }}>
            ...
            <div class="mb-3" id="ipAddressContainer">
                ...
                <input type="text" class="form-control @error('ip_address') is-invalid @enderror" id="ip_address" name="ip_address" value="{{ old('ip_address', $ipAddress) }}" {{ old('entry_option', $defaultEntryOption) == 'automatik' ? 'disabled' : '' }}>
            ...
            <div class="mb-3" id="workProcessSection">
                ...
                <div class="form-check">
                    <input class="form-check-input @error('work_checklist_id') is-invalid @enderror" type="checkbox" id="work_checklist_{{ $workChecklist->id }}" name="work_checklist_id[]" value="{{ $workChecklist->id }}" @if (in_array($workChecklist->id, old('work_checklist_id', $selectedWorkProcesses ?? []))) checked @endif>
                    <label class="form-check-label" for="work_checklist_{{ $workChecklist->id }}">
                        {{ $workChecklist->title }}
                    </label>
                </div>
                @endforeach
                @error('work_checklist_id')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>

            <div class="mb-3" id="vmsNo">
                <label for="vms_no" class="form-label">No. VMS<a href="https://units.uitm.edu.my/aduan_add.cfm" target="_blank" class="ms-2">(VMS)</a></label>
                <input type="text" class="form-control @error('vms_no') is-invalid @enderror" id="vms_no" name="vms_no" value="{{ old('vms_no') ?? ($maintenanceRecord->vms_no ?? '') }}">
                @error('vms_no')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3" id="aduanUnitNo">
                <label for="aduan_unit_no" class="form-label">No. Aduan Unit<a href="https://units.uitm.edu.my/aduan_add.cfm" target="_blank" class="ms-2">(UNITS)</a></label>
                <input type="text" class="form-control @error('aduan_unit_no') is-invalid @enderror" id="aduan_unit_no" name="aduan_unit_no" value="{{ old('aduan_unit_no') ?? ($maintenanceRecord->aduan_unit_no ?? '') }}">
                @error('aduan_unit_no')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('remarks') is-invalid @enderror" id="remarks" name="remarks" rows="3">{{ old('remarks') ?? ($maintenanceRecord->remarks ?? '') }}</textarea>
                @error('remarks')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
