                        ...
                        <tr>
                            ...
                            <td class="col-4">
                                @if (count($userCredentials) > 0)
                                    @php
                                        // Initialize a flag to check if all credentials are empty
                                        $allEmpty = true;

                                        // Iterate through the credentials to verify
                                    ...
                                    @else
                                        @foreach ($userCredentials as $index => $credential)
                                            @if (count($userCredentials) > 1)
                                                <p class="badge bg-primary text-uppercase mb-1">Akaun {{ $index + 1 }}
                                                </p>
                                            @endif
                                            <p class="mb-0"><strong>Nama Pengguna:</strong>
                                                {{ $credential['username'] ?? '<em>Tiada</em>' }}</p>
                                            <p class="mb-0"><strong>Kata Laluan:</strong>
                                                ...
                                                @if ($credential['password'])
                                                    <button type="button" class="btn btn-link toggle-password"
                                                        style="padding: 0; font-size: 1.1rem;">
                                                        <i class="bx bx-show"></i>
                                                    </button>
                                                @endif
                                            </p>
                                            @if ($index < count($userCredentials) - 1)
                                                <hr class="my-1" />
                                            @endif
                                        @endforeach
                                    @endif
