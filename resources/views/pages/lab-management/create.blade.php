        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('lab-management') }}">Rekod Selenggara Makmal Komputer</a>
                    </li>
            ...
            <form method="POST" action="{{ $save_route }}">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select {{ $errors->has('computer_lab_id') ? 'is-invalid' : '' }}" id="computer_lab_id"
                        name="computer_lab_id">
                        <option value="" disabled selected>Pilih Makmal Komputer</option>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control {{ $errors->has('computer_no') ? 'is-invalid' : '' }}" id="computer_no"
                        value="{{ $labManagement->computerLab->no_of_computer ?? 0 }}" disabled>
                    <input type="hidden" name="computer_no" id="hidden_computer_no"
                        value="{{ $labManagement->computerLab->no_of_computer ?? 0 }}">
                </div>

                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="start_time" class="form-label">Masa Mula</label>
                        <input type="datetime-local" class="form-control {{ $errors->has('start_time') ? 'is-invalid' : '' }}"
                            id="start_time" name="start_time" value="{{ now()->format('Y-m-d\TH:i') }}" disabled>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="end_time" class="form-label">Masa Tamat</label>
                        <input type="text" class="form-control {{ $errors->has('end_time') ? 'is-invalid' : '' }}" id="end_time"
                            name="end_time" value="{{ old('end_time') ?? ($maintenanceRecord->end_time ?? '') }}" disabled>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    @foreach ($labCheckList as $labCheck)
                        <div class="form-check">
                            <input class="form-check-input {{ $errors->has('lab_checklist_id') ? 'is-invalid' : '' }}" type="checkbox" id="lab_checklist_{{ $labCheck->id }}"
                                name="lab_checklist_id[]" value="{{ $labCheck->id }}"
                                @if (in_array($labCheck->id, old('lab_checklist_id', $selectedlabChecks ?? []))) checked @endif>
                ...
                <div class="mb-3">
                    ...
                    @foreach ($softwareList as $software)
                        <div class="form-check">
                            <input class="form-check-input {{ $errors->has('software_id') ? 'is-invalid' : '' }}" type="checkbox" id="software_{{ $software->id }}"
                                name="software_id[]" value="{{ $software->id }}"
                                @if (in_array($software->id, old('software_id', $selectedWorkChecklist ?? []))) checked @endif>
