                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ $save_route }}">
                            ...
                            @php
                                ...
                                $canEditAll = auth()->user()->hasAnyRole(['Superadmin', 'Admin', 'Pegawai Penyemak']);
                            @endphp

                            <!-- Name Field -->
                            <div class="mb-3">
                                <label for="name" class="form-label">Nama</label>
                                <input type="text" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" id="name" name="name" value="{{ $user->name }}" {{ $canEditAll || $isPemilik ? '' : 'disabled' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="email" class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" id="email" name="email" value="{{ $user->email }}" {{ $canEditAll || $isPemilik ? '' : 'disabled' }}>
                                @if (!($canEditAll || $isPemilik))
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control {{ $errors->has('staff_id') ? 'is-invalid' : '' }}" id="staff_id" name="staff_id" value="{{ $user->staff_id }}" {{ $canEditAll || $isPemilik ? '' : 'disabled' }}>
                                @if (!($canEditAll || $isPemilik))
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select {{ $errors->has('position_id') ? 'is-invalid' : '' }}" id="position_id" name="position_id" {{ $canEditAll || $isPemilik ? '' : 'disabled' }}>
                                    @foreach ($positionList as $position)
                                        <option value="{{ $position->id }}" {{ old('position_id') == $position->id || ($user->position_id ?? '') == $position->id ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control {{ $errors->has('office_phone_no') ? 'is-invalid' : '' }}" id="office_phone_no" name="office_phone_no" value="{{ old('office_phone_no', $user->office_phone_no) }}" {{ $isPemilik ? 'disabled' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-check">
                                    @foreach ($campusList as $campus)
                                        <input class="form-check-input {{ $errors->has('campus_id') ? 'is-invalid' : '' }}" type="checkbox" name="campus_id[]" value="{{ $campus->id }}" id="campus_{{ $campus->id }}" {{ in_array($campus->id, old('campus_id', $user->campus->pluck('id')->toArray())) ? 'checked' : '' }} {{ $canEditAll || $isPemilik ? '' : 'disabled' }}>
                            ...
                            <button type="submit" class="btn btn-primary" {{ $isPemilik ? 'disabled' : '' }}>Simpan</button>
                        </form>
                    </div>
                </div>
            </div>
